...
export default function AlbumPage() {
  const { user } = useAuth();
  const { userStickers, haveIds, needIds, duplicateIds, markSticker, decrementDuplicate, removeSticker } = useStickers(user?.id);
  ...
  // Stickers filtrados para a aba Álbum
  ...
  // Stickers com repetidas para a aba Repetidas
  ...
  const totalExtras = useMemo(() =>
    duplicateStickers.reduce((sum, s) => {
      const qty = userStickers[s.id]?.quantity ?? 2;
      return sum + (qty - 1);
    }, 0),
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="bg-green-800 text-white px-4 py-3">
        ...
        {/* Abas: Álbum / Repetidas */}
        <div className="flex gap-1 bg-green-900/40 rounded-xl p-1 w-fit">
          <button
            onClick={() => { setAlbumTab('album'); exitSelectMode(); }}
            className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${
              albumTab === 'album' ? 'bg-white text-green-800 shadow' : 'text-green-200 hover:text-white'
            }`}
          >
            📚 Álbum
          </button>
          <button
            onClick={() => { setAlbumTab('duplicates'); exitSelectMode(); }}
            className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-1.5 ${
              albumTab === 'duplicates' ? 'bg-white text-green-800 shadow' : 'text-green-200 hover:text-white'
            }`}
          >
            🔁 Repetidas
            {duplicateIds.length > 0 && (
              <span className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${
                albumTab === 'duplicates' ? 'bg-amber-500 text-white' : 'bg-amber-400 text-white'
              }`}>
                {duplicateIds.length}
              </span>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {duplicateStickers.length === 0 ? (
            <div className="text-center text-gray-400 mt-20">
              ...
              <p className="font-bold text-lg text-gray-600">Nenhuma figurinha repetida</p>
              <p className="text-sm mt-1">Marque uma figurinha como repetida no álbum usando o botão +</p>
            </div>
          ) : (
            <>
              {/* Resumo */}
              <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-5 flex items-center gap-4">
                ...
                <div>
                  <p className="font-extrabold text-amber-800 text-lg">{duplicateIds.length} tipo{duplicateIds.length > 1 ? 's' : ''} com repetição</p>
                  <p className="text-sm text-amber-600">{totalExtras} cópia{totalExtras !== 1 ? 's' : ''} extra disponíve{totalExtras !== 1 ? 'is' : 'l'} para troca</p>
                </div>
              </div>
        ...
        <>
          ...
          <div className="bg-white border-b border-gray-100 px-4 py-2 flex items-center gap-2 overflow-x-auto scrollbar-hide">
            ...
            <input
              ...
              className="bg-gray-100 text-gray-700 placeholder-gray-400 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-green-400 flex-shrink-0 w-28 sm:w-36"
            />
          ...
          {selectMode && (
            <div className="bg-blue-50 border-b border-blue-100 px-4 py-2 flex items-center gap-3">
              ...
              <button onClick={toggleSelectAll} className="ml-auto text-xs font-bold text-blue-600 hover:text-blue-800 underline">
